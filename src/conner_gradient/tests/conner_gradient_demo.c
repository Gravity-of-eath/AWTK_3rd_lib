...
static widget_t* make_gauge(widget_t* win, int32_t x, const char* c0, const char* c1,
  ...
  widget_t* g = conner_gradient_view_create(win, x, 30, GAUGE_SZ, GAUGE_SZ);
...
int main(void) {
  ...
  s_g[0] = make_gauge(win, 10, "#00FF88", "#FF0066", 135, 405, 0.25f, TRUE);
  s_g[1] = make_gauge(win, 225, "#00FF88", "#FF0066", 135, 405, 0.25f, FALSE);
  s_g[2] = make_gauge(win, 440, "#FF0000", "#0000FF", 0, 360, 0.30f, TRUE);

  printf("左=cache开  中=cache关(应与左完全一致)  右=整圆\n");
  s_t0 = now_s();
  timer_add(on_tick, NULL, 16);
