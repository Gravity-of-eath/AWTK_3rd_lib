...
#elif defined(EGL_BACKEND)
#include "EGLEnvironment.h"
typedef EGLBackend Backend; 
#endif

using namespace std;

// Extracts an absolute path to the resources directory given the executable
// path. It assumes that the ressources can be found in "[app
// path]/../../Content". This function implements the platform specific
// differences.
std::string extractResourcePath(char *fullPath) {
  ...
  // Android: Currently all assets are inside the apk in assets/Content folder.
  // SDL searches in assets folder automatically. IOS: Content folder should be
...
SampleApplication::SampleApplication(int argc, char **argv, const char *title,
                                     float fov, float near_plane,
                                     float far_plane, int width, int height,
                                     bool fullscreen, bool offscreen,
                                     bool show_cursor, int benchmark_length)
    : _x(15), _y(3), _z(20), _rx(-10), _ry(60), _velocity(0.1f), _helpRows(12),
      _helpLabels(0), _helpValues(0), _curPipeline(0), _cam(0),
      _initialized(false), _running(false),
      _resourcePath(extractResourcePath(argv[0])),
      _benchmark(checkForBenchmarkOption(argc, argv)),
      _benchmarkLength(benchmark_length), _curFPS(H3D_FPS_REFERENCE),
      //    _winHandle(0),
      _winTitle(title), _initWinWidth(width), _initWinHeight(height),
      _winSampleCount(0), _sampleCount(0), _winFullScreen(fullscreen),
      _winOffScreen(offscreen), _prevMx(0), _prevMy(0),
      _winShowCursor(show_cursor), _winHasCursor(false), _fov(fov),
      _nearPlane(near_plane), _farPlane(far_plane), _statMode(0),
      _freezeMode(0), _renderCaps(0), _renderInterface(0),
...
bool SampleApplication::init() {
  printf("SampleApplication 1 z=%f  x=%f  des=%f\n", des, _z, _x);
  des = sqrt((_z * _z) + (_x * _x));
  degg = 0.0f;
  printf("SampleApplication z=%f  x=%f  des=%f\n", des, _z, _x);
  // Init params can be changed in derived user applications
  auto params = setupInitParameters(defaultRenderInterface());
  if (!_backend->init(params))
    return false;
  ...
  if (!h3dInit((H3DRenderDevice::List)_renderInterface)) {
    ...
    _backend->logMessage(LogMessageLevel::Info,
                         "Trying OpenGL2 render backend");
    ...
    if (!h3dInit((H3DRenderDevice::List)_renderInterface)) {
      _backend->logMessage(LogMessageLevel::Error,
  ...
  if (!h3dCheckExtension("Overlays")) {
    _backend->logMessage(LogMessageLevel::Error,
                         "Unable to find overlays extension");
  ...
  h3dSetOption(H3DOptions::GatherTimeStats,
               1); // Set to 0 to improve performance on low-end machines
  if (params.debugContext)
    h3dSetOption(H3DOptions::DebugRenderBackend, 1);
...
SampleApplication::setupInitParameters(int render_interface) {
  ...
  case (int)RenderAPI::OpenGL4:
    ...
    if (_renderCaps & RenderCapabilities::TessellationShader)
      params.minorVersion = 1;
    if (_renderCaps & RenderCapabilities::ComputeShader)
      params.minorVersion = 3;
    if (_renderCaps & RenderCapabilities::DebugBackend)
      params.debugContext = true;
  ...
  case (int)RenderAPI::OpenGLES3:
    ...
    if (_renderCaps & RenderCapabilities::ComputeShader)
      params.minorVersion = 1;
    if (_renderCaps & RenderCapabilities::GeometryShader)
    ...
    params.offscreen = _winOffScreen;
...
WindowCreateParameters SampleApplication::setupWindowParameters() {
  ...
  winParams.offScreen = _winOffScreen;
  winParams.windowTitle = _winTitle;
  winParams.swapInterval = 0;

  // Override fullscreen option for Android and IOS, because apps are pretty
  // much always fullscreen there
  Platform p = _backend->getPlatform();
  if (p == Platform::Android || p == Platform::IOS)
    winParams.fullScreen = true;
...
void SampleApplication::getSize(int &width, int &height) const {
  if (_winHandle)
    _backend->getSize(_winHandle, &width, &height);
  else {
    width = -1;
    height = -1;
  }
...
void SampleApplication::setFreezeMode(int mode) { _freezeMode = mode % 3; }
...
int SampleApplication::run() {
  ...
  if (_benchmark) {
    ...
    double avgFPS =
        _benchmarkLength /
        (std::chrono::duration_cast<std::chrono::duration<double>>(t - _t0)
             .count());
    ...
    while (std::chrono::duration_cast<std::chrono::duration<double>>(
               ...
               .count() < 5.0) {
      const float ww =
          (float)h3dGetNodeParamI(_cam, H3DCamera::ViewportWidthI) /
          (float)h3dGetNodeParamI(_cam, H3DCamera::ViewportHeightI);
      h3dShowInfoBox((ww - 0.32f) * 0.5f, 0.03f, 0.32f, "Benchmark", 1,
                     &fpsLabel, &fpsValue, _fontMatRes, _panelMatRes);
...
void SampleApplication::mainLoop(void *arg) {
  ...
  if (!app->_benchmark && frames >= 3) {
    ...
    fps = frames /
          (float)(std::chrono::duration_cast<std::chrono::duration<double>>(
                      ...
                      .count());
    if (fps < 5)
      fps = H3D_FPS_REFERENCE; // Handle breakpoints
    frames = 0;
    app->_t0 = t;
  }
  ...
  if (app->_benchmark && frames == app->_benchmarkLength)
    app->requestClosing();
}
...
bool SampleApplication::initResources() {
  ...
  // Pipelines
  _pipelineRes[0] = h3dAddResource(H3DResTypes::Pipeline,
                                   "pipelines/forward.pipeline.xml", 0);
  _pipelineRes[1] = h3dAddResource(
      H3DResTypes::Pipeline, "pipelines/deferred.pipeline.particles.xml", 0);
  _pipelineRes[2] =
      h3dAddResource(H3DResTypes::Pipeline, "pipelines/hdr.pipeline.xml", 0);

  // Overlays
  _fontMatRes =
      h3dAddResource(H3DResTypes::Material, "overlays/font.material.xml", 0);
  _panelMatRes =
      h3dAddResource(H3DResTypes::Material, "overlays/panel.material.xml", 0);
  _logoMatRes =
      h3dAddResource(H3DResTypes::Material, "overlays/logo.material.xml", 0);
  ...
  if (_helpRows > 2) {
    _helpLabels[2] = "F3:";
    _helpValues[2] = "Pipeline (...)";
  }
  if (_helpRows > 3) {
    _helpLabels[3] = "F4:";
    _helpValues[3] = "Debug (ON/OFF)";
  }
  if (_helpRows > 4) {
    _helpLabels[4] = "F5:";
    _helpValues[4] = "Wireframe (ON/OFF)";
  }
  if (_helpRows > 5) {
    _helpLabels[5] = "F6:";
    _helpValues[5] = "MSAA: decrease samples";
  }
  if (_helpRows > 6) {
    _helpLabels[6] = "F7:";
    _helpValues[6] = "MSAA: increase samples";
  }
  if (_helpRows > 7) {
    _helpLabels[7] = "F11:";
    _helpValues[7] = "Fullscreen (ON/OFF)";
  }
  if (_helpRows > 8) {
    _helpLabels[8] = "Esc:";
    _helpValues[8] = "Exit";
  ...
  if (_helpRows > 9) {
    _helpLabels[9] = "Space:";
    _helpValues[9] = "Freeze (...)";
  }
  if (_helpRows > 10) {
    _helpLabels[10] = "W/A/S/D:";
    _helpValues[10] = "Movement";
  }
  if (_helpRows > 11) {
    _helpLabels[11] = "LShift:";
    _helpValues[11] = "Turbo";
  }
...
void SampleApplication::update() {
  degg += 1;
  _x = cosf(H3D_DEG2RAD * (degg)) * des;
  _z = sinf(H3D_DEG2RAD * (degg)) * des;
  _ry = -degg + 90;
}
...
void SampleApplication::render() {
  ...
  if (_statMode > 0) {
    ...
    h3dShowText(piperes_name.c_str(), 0.03f, 0.23f, 0.026f, 1, 1, 1,
                _fontMatRes);
  ...
  // Show help
  if (_showHelpPanel) {
    h3dShowInfoBox(ww - 0.48f, 0.03f, 0.45f, "Help", _helpRows, _helpLabels,
                   _helpValues, _fontMatRes, _panelMatRes);
  }
...
void SampleApplication::mouseMoveHandler(float x, float y, float prev_x,
                                         float prev_y) {
...
void SampleApplication::mousePressHandler(int mouseButton, int mouseButtonState,
                                          int actionCount) {
  if (_freezeMode == 2 || _benchmark)
    return;
...
void SampleApplication::touchEventHandler(int evType, int touchPosX,
                                          int touchPosY, int fingerID) {
  if (_freezeMode == 2 || _benchmark)
    return;
  ...
  case (int)TouchEvents::FingerDown: {
    ...
    for (size_t i = 0; i < fingers.size(); ++i) {
      if (fingers[i].active) {
        ...
        if (fingers[i].fingerID == fingerID || alreadyAdded)
          continue; // Prevent from adding already added finger data
        ...
        alreadyAdded =
            true; // Signal that finger already added in this event handler
  ...
  case (int)TouchEvents::FingerUp: {
    ...
    for (size_t i = 0; i < fingers.size(); ++i) {
      if (fingers[i].fingerID ==
          fingerID) // Check for finger that is no longer on the screen
      {
...
void SampleApplication::multiTouchHandler(int touchPosX, int touchPosY,
