...
static void scale_widget_group(widget_t *parent, ui_tree_node *child_info, float_t scale)
{
  ...
  {
    return;
    logPrint("scale_widget_group line: %d\n", __LINE__);
  }

  int32_t direct_children = widget_count_children(parent);
  // logPrint("scale_widget_group line: %d  direct_children=%d\n", __LINE__, direct_children);
  for (int32_t i = 0; i < direct_children; i++)
  {
...
static void def_on_scroll_vertical(yps_banner_menu_t *parent, widget_t **childrens, int32_t count, int32_t lossing, int32_t next, float_t progress)
{
  ...
  logPrint("def_on_scroll_vertical line:%d\n", __LINE__);
  ...
  if (r_n == NULL)
  {
    logPrint("ERROR: rect_scale_center returned NULL for next item\n");
    free(r_l);
    return;
  }
...
static ret_t on_anim_function(const timer_info_t *timer)
{
  ...
  logPrint("yps_banner_menu_focus_next 32  \n");
  ...
  logPrint("on_anim_function  p=%f current_progress=%d\n", p, yps_banner_menu->current_progress);
...
{
  logPrint("yps_banner_menu_focus_next 2 anim= %d\n", anim);
  if (anim)
  {
    if (yps_banner_menu->on_animation)
    {
      logPrint("AAA banner_menu  focus_change on_animation seek this operate!\n");
      return;
    }
  ...
  else
  {
    ...
    {
      logPrint("ERROR !!! no layout_manager.on_layout function");
    }
  }
}
...
ret_t yps_banner_menu_focus_next(widget_t *widget)
{
  ...
  logPrint("yps_banner_menu_focus_next 1\n");
